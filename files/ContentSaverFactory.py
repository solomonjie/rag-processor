...
class ContentSaver:
    ...
    def save_content(content: str, path: str, storage_type: Optional[str] = None, metadata: Optional[Dict] = None) -> str:
        ...
        try:
            # 3. 路由到具体私有处理方法
            if storage_type == "s3":
                return ContentSaver._save_to_s3(path, payload)
            elif storage_type == "azure":
                return ContentSaver._save_to_azure(path, payload)
            else:
                return ContentSaver._save_to_local(path, payload)
    ...
    @staticmethod
    def _save_to_local(path: str, data: Dict[str, Any]) -> str:
        """保存到本地磁盘"""
        # 确保目录存在
    ...
    @staticmethod
    def _save_to_s3(path: str, data: Dict[str, Any]) -> str:
        """保存到 AWS S3 (示例接口)"""
    ...
    @staticmethod
    def _save_to_azure(path: str, data: Dict[str, Any]) -> str:
        """保存到 Azure Blob (示例接口)"""
